...
> *رد عـلـي الـرسـالـه بـ رقـم الـقـسـم فـقـط بـدون نـقـطـه*`;
    ...
    // تم تعديل quoted إلى global.reply_status || m لتفادي أي خطأ تعريف
...
menu.before = async (m, { conn, bot }) => {
    ...
    if (!cat) {
        await conn.sendMessage(m.chat, { text: '*❌≥ اختار رقم من القائمة بس*' }, { quoted: global.reply_status || m });
        return true;
    }
    
    const cmds = menuData.cats[cat[2]];
    if (!cmds?.length) {
        await conn.sendMessage(m.chat, { text: '*❌≥ القسم فاضي*' }, { quoted: global.reply_status || m });
        return true;
    }
